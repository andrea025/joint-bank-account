  ...
  describe("Withdraw", () => {
    describe("Request a withdraw", () => {
      it("Should allow account owner to request a withdraw", async () => {
        ...
        // if this transaction fails the test case will revert, so it is a valid test case even without assertions
        await bankAccount.connect(addr0).requestWithdrawl(0, 100);
      });

      it("Should allow account owner to request multiple withdraw", async () => {
        const { bankAccount, addr0} = await deployBankAccountWithAccounts(1, 100);
        await bankAccount.connect(addr0).requestWithdrawl(0, 90);
        await bankAccount.connect(addr0).requestWithdrawl(0, 10);
      });

      it("Should not allow account owner to request a withdraw with invalid amount", async () => {
        const { bankAccount, addr0} = await deployBankAccountWithAccounts(1, 100);
    ...
    describe("Approve a withdraw", () => {
      ...
      it("Should not allow creator of the request to approve withdrawl", async () => {
