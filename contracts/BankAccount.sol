...
contract BankAccount {
    ...
    modifier accountOwner(uint256 accountId) {
        ...
        require(isOwner, "Not an owner of this account");
        _;
    }
    ...
    modifier canApprove(uint256 accountId, uint256 withdrawId) {
        require(!accounts[accountId].withdrawRequests[withdrawId].approved, "This request has already been approved");
        require(accounts[accountId].withdrawRequests[withdrawId].user != msg.sender, "The user who made the request cannot approve it");
    ...
    modifier canWithdraw(uint256 accountId, uint256 withdrawId) {
        ...
        require(accounts[accountId].withdrawRequests[withdrawId].approved, "This request has not been approved");
        _;
    }
    ...
    function approveWithdrawl(uint256 accountId, uint256 withdrawId) external accountOwner(accountId) canApprove(accountId, withdrawId) {
        ...
        // the owner of the account who makes the request
        // automatically approves it, that's why - 1
    ...
    function getOwners(uint256 accountId) external view returns (address[] memory) {
        return accounts[accountId].owners;
    }

    function getApprovals(uint256 accountId, uint256 withdrawId) external view returns (uint256) {
